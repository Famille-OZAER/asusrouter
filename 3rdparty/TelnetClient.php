...
class TelnetClient {
	...
	public function exec($command, $add_newline = TRUE) {
		...
		$this->waitPrompt();
		$message=$this->getBuffer();
		log::add('asusrouter', 'debug', $message);
		return $message;
	}
	...
	/**
	 * Set the default options necessaries for sending and reading sms from router
	 */
	public function setDefaultOptions()
	{
		...
		echo "Set prefered storage to SIM"
		/usr/sbin/modem_at.sh +CPMS=\"SM\"*/
		$this->exec('/usr/sbin/modem_at.sh +CSCS=\"IRA\"');
		$this->exec('/usr/sbin/modem_at.sh +CMGF=1');
		$this->exec('/usr/sbin/modem_at.sh +CPMS=\"SM\"');
	}
	/**
